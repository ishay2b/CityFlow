...
class DQN(nn.Module):
    ...
    def __init__(self, n_actions, env, device):
        super(DQN, self).__init__()
        self.env = env
        self.device = device
        w, h = env.MAT_SIZE, env.MAT_SIZE
    ...
    def get_screen(self):
        ...
        image = self.env.quantize_list(locations).reshape(1, 1, self.env.MAT_SIZE, self.env.MAT_SIZE)
...
self = policy_net = DQN(n_actions, env, device).to(device)
target_net = DQN(n_actions, env, device).to(device)
...
get_screen = policy_net.get_screen
...
for i_episode in range(num_episodes):
    ...
    env.reset()
    last_screen = policy_net.get_screen()
    ...
    for t in count():
        ...
        sys.stdout.write(f"\r reward {reward}")
        sys.stdout.flush()
        # Observe new state
    ...
    print(f"\n done episode in {default_timer()-start_time} seconds. reward {reward}")
