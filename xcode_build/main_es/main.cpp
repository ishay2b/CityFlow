...
int main(int argc, const char * argv[]) {
    ...
    std::string config_path = "/Users/ishay/projects/CityFlow/data/atlanta/config_engine.json";
    if (argc > 1) {
    ...
    double avg = 0.0;
    const int num_steps_main = CityFlow::getJsonMember<const int>("num_steps_main", eng.m_config);
    while (step<num_steps_main) {
        auto v = eng.getTrafficLightPhases();
        ...
        total_vc_count += vc;
        double avg_time = eng.getAverageTravelTime();
        ...
        if (step  % 100 == 1){
            ...
            cout<<step<<"   "<<ratio<<"  avg speed    "<<to_string(avg)<<" avg speed:" << to_string(avg_time)<<  "\r"<<std::flush;
